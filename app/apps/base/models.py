...
class SiteSettings(SEOModel):
    ...
    # Питание и аренда оборудования — общие для всех залов, поэтому
    # хранятся один раз, а не дублируются в каждом зале.
    events_extras = models.TextField(
        _('Питание и оборудование для мероприятий'), blank=True,
        help_text=_('Выводится внизу страницы конференц-залов. Каждая строка — '
                    'отдельный пункт; строка вида «Заголовок:» становится подзаголовком.'),
    )
